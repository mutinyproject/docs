...
* Packages
    ...
    ** Toolchain
        ...
        *** https://clang.llvm.org[`clang`] over {gnu}/gcc[`gcc`]
        *** https://libcxx.llvm.org[`libc{pp}`] over `libstdc{pp}`
        *** http://invisible-island.net/byacc/byacc.html[`byacc`] over {gnu}/bison[`bison`]
        *** https://github.com/sabotage-linux/gettext-tiny[`gettext-tiny`] over {gnu}/gettext[`gettext`]

=== Mutiny utilities

* `praxis` - a source-based and binary-based package manager
